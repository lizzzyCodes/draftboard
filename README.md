...
Draftboard presents NBA players as collectible trading cards, front and back, with the back surfacing the metrics and context.
...
**What scouts actually weigh.** Beyond box-score numbers, recurring themes were "82-game value vs. 16-game value" (long-term production vs. playoff-lens fit), positional size and being "over 6'8"," length and defensive versatility, shot selection and aggressiveness, "vision," and flashes of an "NBA move"  skills that look translatable to the next level. Several of these (like shooting touch or an "NBA move") are fundamentally things a stats API cannot capture; they only show up on film or in person. 
...
![NBA Card Inspo](/assets/NBACardInspo.png)
![NBA Card Inspo](/assets/BackofCardInspo.png)


## Architecture

✓ Database serves as the application's single source of truth

✓ Frontend never communicates directly with NBA APIs

✓ API documentation generated automatically with FastAPI
...
- **Framer Motion** (3D card tilt, flip mechanics, holofoil sheen)
- **Lucide React, Google Fonts (`@fontsource`)
...
FastAPI automatically generates interactive documentation accessible directly in the browser:
- **Swagger UI (Interactive Docs):** `https://draftboard-g9uv.onrender.com/docs`
- **ReDoc (Alternative Specs):** `https://draftboard-g9uv.onrender.com/redoc`

#### Endpoints Summary
| Endpoint | Method | Description | Example / Parameters |
| :--- | :--- | :--- | :--- |
| `/players/` | `GET` | Retrieve list of all seeded players | `/players/` |
| `/players/{nba_player_id}` | `GET` | Get detailed player profile (bio, team details, combine measurements, & season/playoff stats) | `/players/1628369` |
| `/teams/` | `GET` | Retrieve list of all 30 NBA teams (ESPN color hex codes & logos) | `/teams/` |
| `/teams/{abbreviation}` | `GET` | Get team details by 3-letter abbreviation | `/teams/BOS` |
| `/shots/{nba_player_id}` | `GET` | Retrieve shot chart court coordinates and make/miss data | `/shots/1628369?season=2023-24` |
...
- "Add a sheen holofoil effect to this card — a diagonal light-refracting band (like Pokémon XY-era holo cards) that sweeps across the surface based on cursor position, plus a subtle 3D tilt. The sheen should feel attached to the card's surface, not like a spotlight chasing the cursor." 
...
## Planned Features (wireframes)
- **Comparison Screen** — when two cards get selected to compare
`
![Comparison Screen](/assets/ComparisonScreen.png)

- **Shot chart visualization** — plotting each player's shot data (already seeded) onto a basketball court SVG, so two players' shooting profiles can be visually compared side by side in the same comparison screen

![Shot Chart](/assets/ShotComparisons.png)
...
![Scout Add](/assets/ScoutAdd.png)
![Scout Form](/assets/ScoutAddInfo.png)